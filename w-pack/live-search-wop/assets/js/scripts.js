...
(function($){
    ...
    $(document).on( 'keyup', '.wpc--ls-element', function(e) {
        if ( e.target && e.target.matches('.wpc--ls-input') ) {
            if ( doAction ) {
    ...
    $(document).on( 'click', function(e) {
        if ( willDestroy !== false ) {
            $('.wpc--ls-focus').removeClass('wpc--ls-focus');

            setTimeout( function() {
                $('.wpc--ls-results').remove();
            }, 250 );
        }
    } );
    ...
    $(document).on( 'click', '.wpc--ls-element-wrap.wpc--active', function(e) {
        if (e.target.classList.contains('wpc--active')) {
            $(this).removeClass('wpc--active').addClass('wpc--preparing');

            rClass( 'wpc--preparing', $(this) );
        }
    } );

    $(document).on( 'click', '.wpc--ls-has-search .wpc--ls-button', function(e) {
        $(this).parent().removeClass('wpc--ls-has-search').find('input:first').val('').trigger('focus');
        _destroy(e);
    } );

    $(document).on( 'click', '.wpc--ls-callout', function(e) {
        $('#'+$(this).attr('data-callout')+'-wrap').addClass('wpc--active');
    } );

    $(document).on( 'click', '.wpc--ls-element-wrap .wpc--ls-result a', function(e) {
        $(this).closest('.wpc--ls-element-wrap').removeClass('wpc--active').addClass('wpc--preparing');

        rClass( 'wpc--preparing', $(this) );
    } );

    $(document).on( 'focusin', '.wpc--ls-element', function(e) {
        if ( e.target && e.target.matches('.wpc--ls-input') ) {
            e.target.classList.add('wpc--ls-focus');
    ...
    $('.wpc--ls-element').mouseover(function(e){
        willDestroy = false;
    });

    $('.wpc--ls-element').mouseleave(function(){
        willDestroy = true;
    });

    $(document).on( 'focusout', '.wpc--ls-element', function(e) {
        if ( willDestroy !== false ) {
            if ( e.target && e.target.matches('.wpc--ls-input') ) {
                e.target.classList.remove('wpc--ls-focus');
    ...
    function _destroy(e) {
        $(e.target.parentNode).find('.wpc--ls-results').remove();
    }

    function _check_field_status(e) {
        if (e.target.value.length===0) {
            e.target.parentNode.classList.remove('wpc--ls-has-search');
        } else {
            e.target.parentNode.classList.add('wpc--ls-has-search');
        }
    }
    ...
    function _do_live_search(e) {
        ...
        var s = [ e.target.value, $(e.target).closest('.wpc--ls-element').attr('data-category'), _see_taxonomies(e.target.value) ];
    ...
    function _build_results(e,f) {
        if ( f.length>0 ) {
            var p = '';
            $.each( f, function(i,o) {
                p += '<div class="wpc--ls-result"><div class="wpc--ls-path">'+o.path+'</div><div class="wpc--ls-title">'+o.title+'<div class="wpc--ls-price">'+o.price+'</div><div class="wpc--ls-image">'+o.image+'</div></div></div>';
            } );

            _destroy(e);
            $(e.target.parentNode).append('<div class="wpc--ls-results">'+p+'</div>');
        }
        else {
            _destroy(e);
            $(e.target.parentNode).append('<div class="wpc--ls-results"><div class="wpc--ls-result">'+ls.localize.notfound+'</div></div>');
        }
    }

    function _ajax(s) {
        var data = {
			action: 'wpc_live_search',
            settings: s,
		};
    ...
    function setCallouts() {
        ...
        callouts = document.body.getElementsByClassName( 'wpc--ls-callout' );
        ...
        for ( let callout of callouts ) {
            ...
            wrap.className = "wpc--ls-element-wrap";
