<?php
	...
	class wop_Product_Filters_Frontend {
		...
		function __construct() {
			...
			self::$version = wop_Product_Filters::$version;
			...
			add_filter( 'wop__add_meta_information_used', array( $this, 'prdctfltr_info' ) );
		...
		function _install_filter() {
			...
			$auto = wop_Product_Filters_Themes::get_theme();
			...
			if ( !empty( self::$options['install']['actions'] ) ) {
				foreach( self::$options['install']['actions'] as $action ) {
					...
					$filter = new wop_Product_Filters_Hooks( $action );
				}
			}
		...
		function localize_scripts() {
			...
			if ( !isset( $prdctfltr_global['init'] ) ) {
				...
				$curr_args = apply_filters( 'prdctfltr_localize_javascript', array(
					...
					'localization' => array(
						'close_filter' => esc_html__( 'Close filter', 'woopack' ),
						'filter_terms' => esc_html__( 'Filter terms', 'woopack' ),
						'ajax_error' => esc_html__( 'AJAX Error!', 'woopack' ),
						'show_more' => esc_html__( 'Show more', 'woopack' ),
						'show_less' => esc_html__( 'Show less', 'woopack' ),
						'noproducts' => esc_html__( 'No products found!', 'woopack' ),
						'clearall' => esc_html__( 'Clear filters', 'woopack' ),
						'getproducts' => esc_html__( 'Show products', 'woopack' ),
						'outofstock' => esc_html__( 'Out of stock', 'woopack' ),
						'backorder' => esc_html__( 'On backorder', 'woopack' ),
					),
					...
					'active_sc' => ( isset( wop_Product_Filters_Shortcodes::$settings['sc'] ) ? wop_Product_Filters_Shortcodes::$settings['sc'] : '' ),
					'animation' => array(
						'delay' => 100,
						'duration' => 300
					),
		...
		public static function make_global( $set, $query = array() ) {
			...
			if ( $stop === false ) {
				...
				if ( !empty( $misc ) || !empty( $rng_for_activated ) || !empty( $mta_for_array ) || count( $taxonomies ) == 1 && !isset( $taxonomies['product_cat'] ) || count( $taxonomies ) > 1 ) {
					add_filter( 'woocommerce_is_filtered', 'wop_Product_Filters_Frontend::return_true' );
				}
				...
				if ( !isset($active['instock_products']) && isset(wop_Product_Filters_Shortcodes::$settings['sc_instock']) && in_array( wop_Product_Filters_Shortcodes::$settings['sc_instock'], array( 'in', 'out', 'both' ) ) ) {
					$active['instock_products'] = wop_Product_Filters_Shortcodes::$settings['sc_instock'];
				}
		...
		public static function sc_wc_query( $query ) {
			if ( isset( $query->query_vars['prdctfltr_active'] ) ) {
				call_user_func( 'wop_Product_Filters_Frontend::prdctfltr_wc_query', $query, array() );
			}
		}
		public static function sc_wc_tax( $query ) {
			if ( isset( $query->query_vars['prdctfltr_active'] ) ) {
				call_user_func( 'wop_Product_Filters_Frontend::prdctfltr_wc_tax', $query, array() );
			}
		}
		...
		public static function prdctfltr_wc_query( $query, $that ) {
			...
			if ( !empty( $pf_activated ) && has_filter( 'post_clauses', array( 'WC_Query', 'price_filter_post_clauses') ) === false ) {
				add_filter( 'posts_clauses', 'wop_Product_Filters_Frontend::price_filter_post_clauses', 10, 2 );
			}
				...
				if ( $cfa > 0 ) {
					...
					if ( !empty( $variableStockOut ) ) {
						...
						if ( isset( $pf_activated['instock_products'] ) && $pf_activated['instock_products'] == 'out' ) {
							add_filter( 'posts_where' , array( 'wop_Product_Filters_Frontend', 'prdctfltr_add_variable_outofstock' ), 99998 );
						}
						else {
		...
		public static function prdctfltr_get_settings() {
			...
			if ( !isset( $prdctfltr_global['mobile'] ) ) {
				if ( !isset( $prdctfltr_global['disable_overrides'] ) || ( isset( $prdctfltr_global['disable_overrides'] ) && $prdctfltr_global['disable_overrides'] !== 'yes' ) ) {
					...
					if ( empty( self::$options['manager'] ) && class_exists( 'wop_Product_Filters_Compatible_Settings' ) ) {
						self::$options['manager'] = wop_Product_Filters_Compatible_Settings::_fix_overrides();
					}
		...
		public static function __check_preset_buttons() {
			...
			if ( self::$settings['instance']['style']['button_position'] == 'top' ) {
				add_action( 'prdctfltr_filter_form_before', 'wop_Product_Filters_Frontend::prdctfltr_filter_buttons', 10 );
				remove_action( 'prdctfltr_filter_form_after', 'wop_Product_Filters_Frontend::prdctfltr_filter_buttons');
			}
			else if ( self::$settings['instance']['style']['button_position'] == 'both' ) {
				add_action( 'prdctfltr_filter_form_after', 'wop_Product_Filters_Frontend::prdctfltr_filter_buttons', 10 );
				add_action( 'prdctfltr_filter_form_before', 'wop_Product_Filters_Frontend::prdctfltr_filter_buttons', 10 );
			}
			else {
				add_action( 'prdctfltr_filter_form_after', 'wop_Product_Filters_Frontend::prdctfltr_filter_buttons', 10 );
				remove_action( 'prdctfltr_filter_form_before', 'wop_Product_Filters_Frontend::prdctfltr_filter_buttons');
			}
				...
				<a class="button prdctfltr_woocommerce_filter_submit" href="#">
					<?php
						if ( self::$settings['instance']['style']['filter_button'] !== '' ) {
							...
							esc_html_e( 'Filter selected', 'woopack' );
						}
					?>
				</a>
			<?php
				...
				<?php
					printf(
						...
						self::$settings['instance']['style']['_tx_sale'] !== '' ? esc_html( self::$settings['instance']['style']['_tx_sale'] ) : esc_html__( 'Show only products on sale' , 'woopack' ),
				...
				<?php
					if ( get_option( 'woocommerce_hide_out_of_stock_items', 'no' ) == 'yes' ) {
						printf(
							...
							esc_html__('Show out of stock products' , 'woopack' ),
						...
						printf(
							...
							self::$settings['instance']['style']['_tx_instock'] !== '' ? esc_html( self::$settings['instance']['style']['_tx_instock'] ) : esc_html__( 'In stock only' , 'woopack' ),
							isset( $pf_activated['instock_products'] ) ? ' class="prdctfltr_active"' : '',
							isset( $pf_activated['instock_products'] ) ? ' checked' : ''
						);
					}
...
<?php
		...
		public static function nice_number( $n ) {
			...
			if ( $n > 1000000000000 ) {
				return round( ( $n / 1000000000000 ) , 1 ).' ' . esc_html__( 'trillion' , 'woopack' );
			}
			else if ( $n > 1000000000 ) {
				return round( ( $n / 1000000000 ) , 1 ).' ' . esc_html__( 'billion' , 'woopack' );
			}
			else if ( $n > 1000000 ) {
				return round( ( $n / 1000000 ) , 1 ).' ' . esc_html__( 'million' , 'woopack' );
			}
			else if ( $n > 1000 ) {
				return round( ( $n / 1000 ) , 1 ).' ' . esc_html__( 'thousand' , 'woopack' );
			}
			...
			if ( self::$filter['title'] != '' ) {
				...
				if ( substr( self::$filter['taxonomy'], 0, 3 ) == 'pa_' ) {
					...
					if ( self::$filter['taxonomy'] == 'product_cat' ) {
						esc_html_e( 'Categories', 'woopack' );
					}
					else if ( self::$filter['taxonomy'] == 'product_tag') {
						esc_html_e( 'Tags', 'woopack' );
					}
					else if ( self::$filter['taxonomy'] == 'characteristics' ) {
						esc_html_e( 'Characteristics', 'woopack' );
					}
					else {
		...
		public static function catalog_instock( $get = '' ) {
			...
			$instock = array(
				'both'    => esc_html__( 'All Products', 'woopack' ),
				'in'      => esc_html__( 'In Stock', 'woopack' ),
				'out'     => esc_html__( 'Out Of Stock', 'woopack' )
			);
		...
		public static function catalog_ordering() {
			...
			$orderby = array(
				'menu_order'       => esc_html__( 'Default', 'woopack' ),
				'comment_count'    => esc_html__( 'Review Count', 'woopack' ),
				'popularity'       => esc_html__( 'Popularity', 'woopack' ),
				'rating'           => esc_html__( 'Average rating', 'woopack' ),
				'date'             => esc_html__( 'Newness', 'woopack' ),
				'price'            => esc_html__( 'Price: low to high', 'woopack' ),
				'price-desc'       => esc_html__( 'Price: high to low', 'woopack' ),
				'rand'             => esc_html__( 'Random Products', 'woopack' ),
				'title'            => esc_html__( 'Product Name', 'woopack' )
			);
		...
		<?php
			if ( self::$settings['instance']['total'] == 0 ) {
				esc_html_e( 'No products found!', 'woopack' );
			}
			else if ( self::$settings['instance']['total'] == 1 ) {
				if ( $pf_step_filter !== '' ) {
					esc_html_e( 'Found a single result', 'woopack' );
				}
				else {
					esc_html_e( 'Showing the single result', 'woopack' );
				}
			}
			else if ( self::$settings['instance']['total'] <= self::$settings['instance']['per_page'] || -1 == self::$settings['instance']['per_page'] ) {
				if ( $pf_step_filter !== '' ) {
					echo esc_html__( 'Found', 'woopack' ) . ' ' . absint( self::$settings['instance']['total'] ) . ' ' . esc_html__( 'results', 'woopack' );
				}
				else {
					echo esc_html__( 'Showing all', 'woopack' ) . ' ' . absint( self::$settings['instance']['total'] ) . ' ' . esc_html__( 'results', 'woopack' );
				}
			}
			else {
				if ( $pf_step_filter !== '' ) {
					...
					echo esc_html__( 'Showing', 'woopack' ) . ' ' . absint( self::$settings['instance']['first'] ) . ' - ' . absint( self::$settings['instance']['last'] ) . ' ' . esc_html__( 'of', 'woopack' ) . ' ' . absint( self::$settings['instance']['total'] ) . ' ' . esc_html__( 'results', 'woopack' );
				}
			}
		?>
			</span>
		...
		?>
			...
			<?php
				if ( self::$settings['instance']['style']['filter_title'] !== '' ) {
					...
					esc_html_e( 'Filter products', 'woopack' );
				}
			?>
				</span>
			<?php
				...
				if ( method_exists( $product, 'is_type' ) && $product->is_type( 'variable' ) ) {
					...
					if ( !empty( self::$settings['v_attr'] ) ) {
						...
						if ( !empty( $varIntersect ) ) {
							...
							$found = wop_Product_Filters_Frontend::prdctrfltr_search_array( $curr_var_set, self::$settings['v_attr'] );
						}
		...
		public static function get_filter_search() {
			...
			$pf_placeholder = self::$filter['placeholder'] != '' ? esc_attr( self::$filter['placeholder'] ) : esc_attr( esc_html__( 'Product keywords', 'woopack' ) );
		...
		public static function _get_filter_meta_range_terms() {
			...
			if ( empty( self::$filter['style']['terms'] ) ) {
				esc_html_e( 'Error! No terms!', 'woopack' );
			}
			else {
			...
			switch( $filter['filter'] ) {
				...
				case 'search' :
					...
					self::$filter = array_merge( array(
						...
						'title'         => esc_html__( 'Search Products', 'woopack' ),
						'desc'          => '',
						'placeholder'   => '',
						'style'         => '',
						'class'         => 'search',
				...
				case 'per_page' :
					...
					self::$filter = array_merge( array(
						'filter'        => $filter['filter'],
						'title'         => esc_html__( 'Per Page', 'woopack' ),
						'desc'          => '',
						'style'         => '',
						'class'         => 'perpage',
				...
				case 'instock' :
					...
					self::$filter = array_merge( array(
						'filter'        => $filter['filter'],
						'title'         => esc_html__( 'Availability', 'woopack' ),
						'desc'          => '',
						'include' => array(
							'relation' => 'IN',
							'selected' => array()
				...
				case 'sort' :
					...
					self::$filter = array_merge( array(
						'filter'        => $filter['filter'],
						'title'         => esc_html__( 'Order By', 'woopack' ),
						'desc'          => '',
						'include' => array(
							'relation' => 'IN',
							'selected' => array()
				...
				case 'vendor' :
					...
					self::$filter = array_merge( array(
						'filter'        => $filter['filter'],
						'title'         => esc_html__( 'Vendor', 'woopack' ),
						'desc'          => '',
						'include' => array(
							'relation' => 'IN',
							'selected' => array()
				...
				case 'price' :
					...
					self::$filter = array_merge( array(
						'filter'        => $filter['filter'],
						'title'         => esc_html__( 'Price', 'woopack' ),
						'desc'          => '',
						'include' => array(
							'relation' => 'IN',
							'selected' => array()
...
<?php
		...
		public static function __get_none_string() {
			return isset( self::$filter['none_text'] ) && !empty( self::$filter['none_text'] ) ? esc_html( self::$filter['none_text'] ) : apply_filters( 'prdctfltr_none_text', esc_html__( 'None', 'woopack' ) );
		}
		...
		function cleanup() {
			remove_filter( 'woocommerce_is_filtered', 'wop_Product_Filters_Frontend::return_true' );
		}

	}

	add_action( 'woocommerce_init', array( 'wop_Product_Filters_Frontend', 'init' ) );
