<?php
	...
	class wop_Product_Filters_Shortcodes_AJAX_Fix {
		function in_admin() {
			return false;
		}
	}

	class wop_Product_Filters_Shortcodes {
		...
		public static function _reset_wcsc_loop() {
			...
			remove_filter( 'the_posts', 'wop_Product_Filters_Shortcodes::get_wp_query' );
			remove_filter( 'loop_start', 'wop_Product_Filters_Shortcodes::get_wp_query' );
			remove_filter( 'pre_get_posts', 'wop_Product_Filters_Frontend::sc_wc_query' );
			remove_filter( 'parse_tax_query', 'wop_Product_Filters_Frontend::sc_wc_tax' );
			remove_filter( 'woocommerce_shortcode_products_query', 'wop_Product_Filters_Shortcodes::fix_wcsc_parameters', 9999, 3 );
		...
		public static function get_wp_query_320( $posts, $query ) {
			...
			wop_Product_Filters_Frontend::$options['sc_instance'] = array(
				'paged'			=> $paged,
				'per_page'		=> $per_page,
				'total'			=> $found,
...
<?php
				...
				if ( !is_ajax() ) {
					if ( self::$settings['wcsc_products'] == 'yes' ) {
						include( wop_Product_Filters_Frontend::$dir . 'templates/product-filter.php' );
			...
			if ( !empty( $atts['prdctfltr'] ) && in_array( $atts['prdctfltr'], array( 'yes', 'widget', 'aside' ) ) ) {
				...
				if ( !is_ajax() ) {
					...
					if ( !isset( $prdctfltr_global['done_filters'] ) ) {
						wop_Product_Filters_Frontend::make_global( $_REQUEST, 'FALSE' );
					}
				}

				add_filter( 'found_posts', 'wop_Product_Filters_Shortcodes::get_wp_query_320', 999, 2 );

				add_filter( 'pre_get_posts', 'wop_Product_Filters_Frontend::sc_wc_query' );
				add_filter( 'parse_tax_query', 'wop_Product_Filters_Frontend::sc_wc_tax' );
		...
		public static function prdctfltr_sc_products_700( $atts, $content = null ) {
			...
			if ( !is_ajax() /*&& !isset( $prdctfltr_global['done_filters'] )*/ ) {
				...
				wop_Product_Filters_Frontend::make_global( self::$settings['sc_ajax_filters'], 'AJAX' );
			}
			...
			if ( $atts['show_products'] == 'no' ) {
				...
				wop_Product_Filters_Frontend::$options['step_filter'] = true;
			}
			else {
				if ( !isset( self::$settings['sc'] ) ) {
					self::$settings['sc'] = $prdctfltr_global['pagefilters'][$prdctfltr_global['unique_id']];
			...
			if ( !is_ajax() ) {
				...
				$prdctfltr_global['preset'] = null;
				$prdctfltr_global['action'] = null;
				wop_Product_Filters_Frontend::$settings['maxheight'] = null;
			}
		...
		public static function prdctfltr_respond_550() {
			...
			if ( isset( $atts['wcsc'] ) ) {
				...
				wop_Product_Filters_Frontend::make_global( $curr_filters, 'AJAX' );
				...
				$data['query'] = self::get_query_string( $curr_filters );
				$data['products'] = call_user_func( 'wop_Product_Filters_Shortcodes::prdctfltr_sc_products_700', $atts['atts'] );
				if ( isset( self::$settings['paginationExport'] ) && is_string( self::$settings['paginationExport'] ) ) {
					$data['pagination'] = self::$settings['paginationExport'];
				}
			...
			if ( $opt['pf_step'] == 0 ) {
				...
				$ajaxTemplates = wop_Product_Filters_Frontend::$options['install']['ajax']['dont_load'];
				...
				if ( !empty( $wp_query ) ) {
					...
					if ( !in_array( 'result', $ajaxTemplates ) && isset( $opt['pf_count_template'] ) ) {
						...
						if ( $wp_query->found_posts > 0 ) {
							wc_set_loop_prop( 'total', intval( wop_Product_Filters_Frontend::$options['sc_instance']['total'] ) );
							wc_set_loop_prop( 'per_page', intval( wop_Product_Filters_Frontend::$options['sc_instance']['per_page'] ) );
							wc_set_loop_prop( 'current_page', max( 1, intval( wop_Product_Filters_Frontend::$options['sc_instance']['paged'] ) ) );
				...
				if ( !in_array( 'orderby', $ajaxTemplates ) && isset( $opt['pf_orderby_template'] ) ) {
					...
					if ( !isset( $_GET['orderby'] ) && isset( $prdctfltr_global['active_filters']['orderby'] ) ) {
						...
						$orderby  = wop_Product_Filters_Frontend::get_catalog_ordering_args();
		...
		public static function get_query_string( $curr_filters ) {
			...
			if ( wop_Product_Filters_Frontend::$options['install']['ajax']['permalinks'] == 'yes' ) {
				return '';
			}
			...
			if ( $opt['pf_set'] == 'shortcode' || wop_Product_Filters_Frontend::$options['install']['ajax']['permalinks'] == 'query' ) {
			...
			if ( get_option( 'permalink_structure' ) !== '' ) {
				...
				if ( isset( $current->taxonomy ) && isset( $curr_filters[$current->taxonomy] ) ) {
					...
					if ( $rewrite !== false ) {
						...
						$redirect = get_permalink( wop_Product_Filters_Frontend::prdctfltr_wpml_get_id( wc_get_page_id( 'shop' ) ) );
		...
		public static function make_query_700() {
			...
			add_filter( 'pre_get_posts', 'wop_Product_Filters_Frontend::sc_wc_query', 999999, 1 );
			add_filter( 'parse_tax_query', 'wop_Product_Filters_Frontend::sc_wc_tax', 999999, 1 );

			$products = new WP_Query( $query_args );

			WC()->query->remove_ordering_args();
			remove_filter( 'pre_get_posts', 'wop_Product_Filters_Frontend::sc_wc_query', 999999 );
			remove_filter( 'parse_tax_query', 'wop_Product_Filters_Frontend::sc_wc_tax', 999999 );
		...
		public static function get_pagination( $pagination ) {
			...
			if ( ( $custom_pagination = wop_Product_Filters_Frontend::$options['install']['ajax']['pagination_function'] ) !== '' ) {
			...
			// if ( is_ajax() ) {
			// 	$GLOBALS['current_screen'] = new wop_Product_Filters_Shortcodes_AJAX_Fix;
			// }

			$args = array(
				'total' => wop_Product_Filters_Frontend::$options['sc_instance']['max_num_pages'],
				'current' => wop_Product_Filters_Frontend::$options['sc_instance']['paged']
			);
			...
			if ( isset( $pagination['sc'] ) ) {
				...
				if ( $prdctfltr_global['pagination_type'] == 'default' ) {
					...
					wop_Product_Filters_Frontend::$settings['template'] = 'loop/pagination.php';
					include( wop_Product_Filters_Frontend::$dir . 'templates/getright.php' );
				}
				...
				$ajax = 'yes';
				$prdctfltr_global['pagination_type'] = wop_Product_Filters_Frontend::$options['install']['ajax']['pagination_type'];
	...
	add_action( 'init', array( 'wop_Product_Filters_Shortcodes', 'init' ), 999 );
