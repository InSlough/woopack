...
class wop_Product_Filters_Compatible_Preset {
	...
	public static function fix_options_group( $options ) {
		...
		foreach( $options as $option ) {
			$fixed[$option[0]] = isset( $option['function'] ) ? call_user_func( 'wop_Product_Filters_Compatible_Preset::' . $option['function'], $option ) : self::___u( self::$preset[$option[1]] );
		}
	...
	public static function __get_terms_from_options_for_per_page() {
		...
		for ( $i = 1; $i <= $perpage_limit; $i++ ) {
			$array[] = array(
				...
				'slug' => 'c' . ($i-1),
				'value' => $perpage_set*$i,
				'title' => $perpage_set*$i . ' ' . ( self::___u( self::$preset['wc_settings_prdctfltr_perpage_label'] ) == '' ? esc_html__( 'Products', 'woopack' ) : self::$preset['wc_settings_prdctfltr_perpage_label'] ),
			);
		}
