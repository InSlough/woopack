...
if ( !class_exists( 'wop_Product_Filters' ) ) :

	final class wop_Product_Filters {
		...
		function __get_default_options() {
			...
			if ( empty( self::$settings ) && get_option( 'wc_settings_prdctfltr_version', false ) !== false ) {
				...
				self::$settings = wop_Product_Filters_Compatible_Settings::fix_options();

				if ( empty( self::$settings['manager'] ) && class_exists( 'wop_Product_Filters_Compatible_Settings' ) ) {
					self::$settings['manager'] = wop_Product_Filters_Compatible_Settings::_fix_overrides();
				}
			}

			include_once( 'includes/class-default.php' );
			self::$settings = wop_Product_Filters_Defaults::__get_default_autoload( self::$settings );
		...
		public function ___get_preset( $key ) {
			...
			if ( isset( $key ) && is_string( $key ) ) {
				...
				if ( $option === false && get_option( 'wc_settings_prdctfltr_version', false ) !== false ) {
					...
					$option = wop_Product_Filters_Compatible_Preset::fix_preset( $preset );
				}
			...
			include_once( 'includes/class-default.php' );
			$option = wop_Product_Filters_Defaults::__get_default_preset( $option );
		...
		public function register_taxonomy( $taxonomy ) {
			...
			if ( !empty( $taxonomy_slug ) && !taxonomy_exists( $taxonomy_slug ) ) {
				$labels = array(
					...
					'search_items'               => esc_html__( 'Search', 'woopack' ) . ' ' . $taxonomy_plural,
					'popular_items'              => esc_html__( 'Popular', 'woopack' ) . ' ' . $taxonomy_plural,
					'all_items'                  => esc_html__( 'All', 'woopack' ) . ' ' . $taxonomy_plural,
					'parent_item'                => null,
					'parent_item_colon'          => null,
					'edit_item'                  => esc_html__( 'Edit', 'woopack' ) . ' ' . $taxonomy_plural,
					'update_item'                => esc_html__( 'Update', 'woopack' ) . ' ' . $taxonomy_plural,
					'add_new_item'               => esc_html__( 'Add New', 'woopack' ) . ' ' . $taxonomy_single,
					'new_item_name'              => esc_html__( 'New', 'woopack' ) . ' ' . $taxonomy_single . ' ' . esc_html__( 'name', 'woopack' ),
					'separate_items_with_commas' => esc_html__( 'Separate', 'woopack' ) . ' ' . $taxonomy_plural . ' ' . esc_html__( 'with commas', 'woopack' ),
					'add_or_remove_items'        => esc_html__( 'Add or remove', 'woopack' ) . ' ' . $taxonomy_plural,
					'choose_from_most_used'      => esc_html__( 'Choose from the most used', 'woopack' ) . ' ' . $taxonomy_plural,
					'not_found'                  => esc_html__( 'No', 'woopack' ) . ' ' . $taxonomy_plural . ' ' . esc_html__( 'found', 'woopack' ),esc_html__( 'No Characteristics found', 'woopack' ),
	...
	function Prdctfltr() {
		return wop_Product_Filters::instance();
	}

	wop_Product_Filters::instance();
